...
/obj/item/device/communicator/ui_interact(mob/user, ui_key = "main", var/datum/nanoui/ui = null, var/force_open = 1)
	...
	for(var/mob/observer/ghost/O in known_devices)
	...
	//Ghosts we invited.
	for(var/mob/observer/ghost/O in voice_invites)
	...
	for(var/mob/observer/ghost/O in voice_requests)
		if(O.exonet && O.client)
	...
	for(var/mob/observer/ghost/ghost in im_contacts)
		if(ghost.exonet)
...
/mob/observer/ghost/receive_exonet_message(origin_atom, origin_address, message, text)
	if(message == "voice")
...
/obj/item/device/communicator/proc/request_im(var/atom/candidate, var/origin_address, var/text)
	var/who = null
	if(isobserver(candidate))
		var/mob/observer/ghost/ghost = candidate
		who = ghost
...
/mob/observer/ghost/verb/join_as_voice()
	set category = "Ghost"
...
/mob/observer/ghost/verb/text_communicator()
	set category = "Ghost"
...
/mob/observer/ghost/verb/show_text_messages()
	set category = "Ghost"
