...
/datum/weather_holder/eros
	temperature = T0C
	allowed_weather_types = list(
		WEATHER_CLEAR		= new /datum/weather/eros/clear(),
		WEATHER_OVERCAST	= new /datum/weather/eros/overcast(),
		WEATHER_LIGHT_SNOW	= new /datum/weather/eros/light_snow(),
		WEATHER_SNOW		= new /datum/weather/eros/snow(),
		WEATHER_BLIZZARD	= new /datum/weather/eros/blizzard(),
		WEATHER_RAIN		= new /datum/weather/eros/rain(),
		WEATHER_STORM		= new /datum/weather/eros/storm(),
		WEATHER_HAIL		= new /datum/weather/eros/hail(),
		WEATHER_BLOOD_MOON	= new /datum/weather/eros/blood_moon()
		)
	planetary_wall_type = /turf/unsimulated/wall/planetary/eros
...
/datum/weather/eros/snow/process_effects()
...
/datum/weather/eros/blizzard/process_effects()
	for(var/turf/simulated/floor/outdoors/snow/S in outdoor_turfs)
		for(var/dir_checked in cardinal)
			var/turf/simulated/floor/T = get_step(S, dir_checked)
			if(istype(T))
...
/datum/weather/eros/rain/process_effects()
...
/datum/weather/eros/hail/process_effects()
	for(var/mob/living/L in living_mob_list_)
		if(L.z in holder.our_planet.expected_z_levels)
			var/turf/T = get_turf(L)
			if(!T.outdoors)
			...
			to_chat(L, "<span class='warning'>The hail raining down on you [L.can_feel_pain(target_zone) ? "hurts" : "damages you"]!</span>")

/datum/weather/eros/blood_moon
	name = "blood moon"
	light_modifier = 0.5
	light_color = "#FF0000"
	transition_chances = list(
